...
{	// From file
	...
	{	// UART: COM1,COM2.....
		...
		{
			printf("Unknow COM port setting ![%s]\n",buffer);
			ret = -1;
		}
	}
	else
...
int RU25Lib::ReadResponse(char* datas)
{
	...
	{		
		...
		{
			if(rj45->ReadtoCRLF(&nbyte, datas + totalByte +1, 5000) == true)
			{
				memset(CompareStr,'\0',sizeof(CompareStr));
				memcpy(CompareStr,datas + totalByte,nbyte);
				totalByte += nbyte;
...
bool RU25Lib::ReadOnePacket(char* datas)
{
	...
	while(1)
	{
		...
		{
			case UART_CONNECT:		
				if (serial.PortRead2CRLF(&nbyte,datas,5000) == true)	//4s timeout
				{
					return true;
				}
				break;
			case RJ45_CONNECT:			
				if(rj45->ReadtoCRLF(&nbyte,datas,4000) == true)
				{
					return true;
				}
				break;
...
int RU25Lib::Inventory(char* result)
{
	int  i,ret = 0;
	char packet[256];
	int  index = 0, len = 0;
...
void RU25Lib::GetNearestEPC(char* EPC)
{
	int i;
	int MaxRssiIndex = 0;
